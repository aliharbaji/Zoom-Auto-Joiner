...
#   *** gathering data from data sheet ***
...
for i in range(2, mondaySheet.max_row + 1):
    arr = [str(mondaySheet.cell(row=i, column=1).value), str(mondaySheet.cell(row=i, column=2).value), str(mondaySheet.cell(row=i, column=3).value), str(mondaySheet.cell(row=i, column=4).value), "0"]
    mondayArray.append(arr)

for i in range(2, tuesdaySheet.max_row + 1):
    arr = [str(tuesdaySheet.cell(row=i, column=1).value), str(tuesdaySheet.cell(row=i, column=2).value), str(tuesdaySheet.cell(row=i, column=3).value), str(tuesdaySheet.cell(row=i, column=4).value), "0"]
    tuesdayArray.append(arr)

for i in range(2, wednesdaySheet.max_row + 1):
    arr = [str(wednesdaySheet.cell(row=i, column=1).value), str(wednesdaySheet.cell(row=i, column=2).value), str(wednesdaySheet.cell(row=i, column=3).value), str(wednesdaySheet.cell(row=i, column=4).value), "0"]
    wednesdayArray.append(arr)

for i in range(2, thursdaySheet.max_row + 1):
    arr = [str(thursdaySheet.cell(row=i, column=1).value), str(thursdaySheet.cell(row=i, column=2).value), str(thursdaySheet.cell(row=i, column=3).value), str(thursdaySheet.cell(row=i, column=4).value), "0"]
    thursdayArray.append(arr)

for i in range(2, fridaySheet.max_row + 1):
    arr = [str(fridaySheet.cell(row=i, column=1).value), str(fridaySheet.cell(row=i, column=2).value), str(fridaySheet.cell(row=i, column=3).value), str(fridaySheet.cell(row=i, column=4).value), "0"]
...
while on:
    ...
    for i in range(len(week[weekday])):  # foreach class of your classes
        ...
        if not isStarted:
            ...
            if hour == int(week[weekday][i][1].split(":")[0]) and minute + t / 60 >= int(
                    week[weekday][i][1].split(":")[1]) and week[weekday][i][4] == "0":
                webbrowser.open("https://edu-il.zoom.us/j/" + week[weekday][i][0])  # opens the Zoom link
        ...
        if isStarted:
            if hour == int(week[weekday][i][2].split(":")[0]) and minute >= int(week[weekday][i][2].split(":")[1]) and \
                    week[weekday][i][4] == "1":
        ...
        if week[weekday][len(week[weekday]) - 1][4] == "1" or hour > int(
                week[weekday][len(week[weekday]) - 1][2].split(":")[
    ...
    if not on:  # check if the program is still working, if not: eliminate(break)
        break
